...
async function generateDocsIndex() {
  ...
  // Exclude entity subdirectory READMEs; we only keep the root consolidated README
  docFiles.push(...componentsReadmes.filter(p => {
    const normalized = p.replace(/\\/g, '/');
    return !normalized.includes('components/dashboard/entities/addresses/README.md') &&
           !normalized.includes('components/dashboard/entities/companies/README.md') &&
           !normalized.includes('components/dashboard/entities/projects/README.md');
  }));
