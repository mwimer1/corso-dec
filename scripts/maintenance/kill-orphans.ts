...
function getProcessesWindows(): ProcessInfo[] {
  ...
  try {
    ...
    // Note: -Filter doesn't support OR, so use Where-Object instead
    const psScript = `Get-CimInstance Win32_Process | Where-Object { $_.Name -eq 'node.exe' -or $_.Name -eq 'deno.exe' } | Select-Object ProcessId, CommandLine, CreationDate, ParentProcessId | ConvertTo-Json -Compress`;
