...
function categorize(baseName: string): string {
  ...
  if (['logger', 'exec', 'lastUpdated', 'safe-match'].some((k) => baseName.includes(k))) return 'File Processing';
  if (baseName.includes('validation-common')) return 'Validation';
  if (['gen-type-audit', 'fix-conditional-warnings'].some((k) => baseName.includes(k))) return 'Specialized';
  return 'Misc';
}
