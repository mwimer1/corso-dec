...
console.log('🔍 Auditing GitHub Actions workflows for secrets usage...');
...
- **Fork Protection**: ${hasForkProtection ? '✅ Implemented' : '❌ Missing'}
- **Security Level**: ${hasForkProtection ? '🟢 Secure' : '🟡 Needs Review'}
...
${secretWorkflows === 0 ? '**🟢 Excellent (No secrets used)**' : 
  protectedWorkflows === secretWorkflows ? '**🟢 Excellent (All secret workflows protected)**' : 
  protectedWorkflows > (secretWorkflows / 2) ? '**🟡 Good (Most secret workflows protected)**' : 
  '**🔴 Needs Improvement (Many unprotected secret workflows)**'}
...
console.log('');
console.log(`📄 Full report generated: ${REPORT_FILE}`);
