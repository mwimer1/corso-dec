 ...
 export function SidebarTooltip({ children, label, side = 'right', delayMs = 80, id }: Props) {
  ...
  const show = () => {
    ...
    timer.current = (window.setTimeout(() => {
      ...
      const gap = 12; // visual spacing (matches --sb-tip-gap token)
      ...
      {layer && open && createPortal(
        <span
          ...
          className={cn(
            ...
            "whitespace-nowrap",
            "font-[var(--sb-tip-font)] text-[var(--sb-tip-size)] leading-[var(--sb-tip-line)] font-medium",
            "text-[var(--sb-tip-fg)] bg-[var(--sb-tip-bg)]",
            "border border-[var(--sb-tip-border)]",
