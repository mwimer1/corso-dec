...
export function SidebarUserProfile({
  ...
  const userProfileContent = (
    <div className={cn(
      ...
      collapsed && 'justify-center px-3 py-3' // Collapsed padding (consistent 12px horizontal)
    )}>
  ...
  if (collapsed) {
    const tooltipLabel = userName ? `${userName} • ${planLabel}` : planLabel;
    return (
      <SidebarTooltip label={tooltipLabel} side="right">
        {userProfileContent}
      </SidebarTooltip>
    );
  }
