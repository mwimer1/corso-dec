...
>(function DashboardLayout({ className, children, contentWidth = "dashboard", ...props }, ref) {
  ...
  // Auto-detect entity routes and use full-width layout for them
  const isEntityRoute = pathname?.match(/^\/dashboard\/(projects|companies|addresses)$/);
  const effectiveContentWidth = contentWidth === "full" || isEntityRoute ? "full" : contentWidth;
  const maxWidthVariant = effectiveContentWidth === "full" ? "none" : "default";
