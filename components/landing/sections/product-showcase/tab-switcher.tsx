      ...
      >
        {normalizedTabs.map((item, idx) => {
          ...
          if (layout === 'grid') {
            ...
            // Add vertical separators: right border on items except last in each row
            // Mobile (2 cols): border on items 0, 2 (even indices) - separator between columns
            // Desktop (4 cols): border on items 0, 1, 2 - separator between all but last
            const isEvenIndex = idx % 2 === 0; // Items 0, 2
            const isNotLastDesktop = idx < 3; // Items 0, 1, 2
            return (
              <div key={isObjectTabs ? item.id : idx} className={cn(
                "relative w-full",
                // Vertical separator: right border lighter than top/bottom borders for subtle separation
                // Mobile: only on even indices (0, 2) to separate the 2 columns
                isEvenIndex && "border-r border-border/50",
                // Desktop: on all items except last (0, 1, 2) to separate the 4 columns
                isNotLastDesktop && "lg:border-r lg:border-border/50",
                // Background matches button so gap separators show tablist bg-border/40
                // Active tab uses full muted background for visible contrast
                ...
                <button
                  ...
                  className={cn(
                    ...
                    // Override height with CSS module for doubled height (580px)
                    // Removed h-full to allow explicit height from CSS module
                    styles['showcaseTabButton'],
