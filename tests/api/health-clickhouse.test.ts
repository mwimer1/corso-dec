...
import * as clickhouseHealthAlias from '@/app/api/health/clickhouse/route';
...
describe('ClickHouse health route alias (/api/health/clickhouse)', () => {
  beforeEach(() => {
    vi.clearAllMocks();
    mockGetClickHouseClient.mockReturnValue({
      query: mockQuery,
  ...
  it('declares Node.js runtime', () => {
    expect(clickhouseHealthAlias.runtime).toBe('nodejs');
  });

  it('GET returns healthy status (same as canonical)', async () => {
    mockQuery.mockResolvedValue([]);

    const res = await clickhouseHealthAlias.GET(new Request('http://localhost/api/health/clickhouse'));
    expect(res.status).toBe(200);
  ...
  it('HEAD returns 204 (same as canonical)', async () => {
    ...
    const res = await clickhouseHealthAlias.HEAD();
    expect(res.status).toBe(204);
    expect(mockQuery).toHaveBeenCalledWith({ query: 'SELECT 1' });
  });
});
