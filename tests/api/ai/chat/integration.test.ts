...
describe('POST /api/v1/ai/chat - Integration', () => {
  ...
  it('should support structured table payload in chunk types', () => {
    // Verify that the AIChunk type supports structured table fields
    // This is a compile-time check - if types are wrong, TypeScript will error
    const chunkWithTable = {
      assistantMessage: {
    ...
    expect(chunkWithTable.assistantMessage.visualizationType).toBe('table');
    expect(chunkWithTable.assistantMessage.tableColumns).toHaveLength(2);
    expect(chunkWithTable.assistantMessage.tableData).toHaveLength(2);
  });
});
