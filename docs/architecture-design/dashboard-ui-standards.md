...
### Mobile Drawer Pattern (≤767px)
- **Default state**: Sidebar collapsed (closed) by default to avoid blocking content
- **Drawer behavior**: Opens as fixed overlay with backdrop blur when hamburger button clicked
- **Closing mechanisms**:
  - Backdrop click closes drawer
  - ESC key closes drawer
  - Route navigation auto-closes drawer (prevents stuck open state)
- **Visual styling**: Fixed overlay (`fixed left-0 top-0 z-40`) with backdrop (`bg-black/50 backdrop-blur-sm`)
- **Breakpoint**: Mobile behavior activates at `max-width: 767px` (Tailwind `md` breakpoint)

### Desktop Behavior (≥768px)
- Sidebar uses inline collapsed/expanded behavior (no overlay)
- Sidebar state persists across route changes
- No backdrop or drawer pattern
...
- **Required imports** (in `app/layout.tsx`):
  ...
  3. `@/styles/tokens/ag-grid.css` (Corso custom overrides)

### Theme Configuration
- **Theme source**: `lib/vendors/ag-grid.theme.ts` (uses `themeQuartz.withParams`)
- **CSS overrides**: `styles/tokens/ag-grid.css` (selection column, density modes, numeric alignment)
...
### Numeric Column Alignment & Formatting
- **Right-alignment**: Numeric columns (currency, number formats) are right-aligned for better readability
- **Implementation**: Column adapter (`lib/entities/adapters/aggrid.ts`) applies `cellClass: 'corso-numeric-cell'` for numeric formats
- **CSS styling**: `styles/tokens/ag-grid.css` contains `.corso-numeric-cell` rules for right-alignment
- **Formatting patterns**:
  - **Currency**: Uses `Intl.NumberFormat` for consistent formatting (e.g., `$12,345.67`)
  - **Numbers**: Standard number formatting with thousand separators (e.g., `12,345`)
  - **CSV export**: Uses raw numeric values (unformatted) or consistent export formatting
- **Supported formats**: `currency`, `number` (both apply right-alignment via `corso-numeric-cell` class)
...
### DashboardLayout Content Width

The `DashboardLayout` component supports two content width modes:

- **`"dashboard"`** (default): Centered content with 1600px max-width for optimal readability on wide screens
- **`"full"`**: Full-width layout for dense grid views or pages requiring edge-to-edge content

**Usage:**
```tsx
// Default: centered content (1600px max-width)
<DashboardLayout>
  <YourPageContent />
</DashboardLayout>
...
**Implementation:**
- Default uses `dashboardShellVariants({ maxWidth: 'default' })` → `max-w-[1600px]` from `containerMaxWidthVariants`
- Full-width uses `dashboardShellVariants({ maxWidth: 'none' })` for edge-to-edge
- See `styles/ui/organisms/dashboard-shell.ts` and `styles/ui/shared/container-base.ts` for variant definitions

**When to use full-width:**
- Entity grids (projects, companies, addresses) that need maximum horizontal space
- Dense data tables with many columns
- Pages explicitly designed for wide-screen consumption

**When to use default (dashboard):**
- Standard dashboard pages with forms, cards, or mixed content
- Content that benefits from constrained width for readability
- Pages with text-heavy content
