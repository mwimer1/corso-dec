...
- [Testing Guide](../quality/testing-guide.md) - Test environment configuration
...
```typescript
// ALLOWED: NODE_ENV check for dev-only logging (build-time optimization exception)
if (process.env.NODE_ENV !== 'production') {
  console.log('Debug information');
}

// ALLOWED: NODE_ENV check for dev-only validation (build-time optimization exception)
if (process.env.NODE_ENV !== 'production') {
...
```typescript
// ALLOWED: NODE_ENV check for runtime compatibility (build-time optimization exception)
