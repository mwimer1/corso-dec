...
- `pnpm validate:dead-code:optimized` - **Recommended**: Combines orphan + cycle detection with tooling-only import detection (~50% faster)
- `pnpm validate:ts-unused` - Find unused TypeScript exports
- `pnpm validate:dead-code:all` - Alias for `validate:dead-code:optimized` (recommended)
...
- **Detect Tooling-Only Imports**: Automatically identifies files only imported by tooling configs (e.g., `tailwind.config.ts`) - shown as warnings, not errors
...
#### `validate:dead-code:optimized` (Recommended)
```bash
pnpm validate:dead-code:optimized
# Combines orphan and cycle detection in a single pass
# ~50% faster than running separately
# Includes tooling-only import detection
```

**What it does:**
- Combines orphan and cycle detection using a single Madge pass
- Detects **tooling-only imports** (files only imported by tooling configs)
- Builds full dependency graph to trace import chains
- Reports tooling-only files as warnings (not errors)
- Fails CI only on real orphans or cycles

**Tooling-Only Import Detection:**
- Identifies files imported only by tooling configs (e.g., `tailwind.config.ts`, `postcss.config.ts`)
- These are "false orphans" because tooling configs are excluded from orphan scans
- Shows import chain information for debugging
- Tooling-only files are informational (warnings) and don't fail CI
- If intentional, add to `scripts/audit/orphans.allowlist.json`

**Example Output:**
```
⚠️  Found 3 file(s) imported ONLY by tooling configs (false orphans):
   - styles/breakpoints.ts
     Imported by: tailwind.config.ts
     Import chain example: tailwind.config.ts → styles/breakpoints.ts

❌ Found 2 orphaned file(s) (1 allowlisted):
   - lib/unused-util.ts
   - components/deprecated-button.tsx
```

#### `validate:dead-code:all`
```bash
pnpm validate:dead-code:all
# Alias for validate:dead-code:optimized
# Runs optimized validation (recommended)
```

**What it does:**
- Alias for `validate:dead-code:optimized`
- Combines orphan and cycle detection efficiently
- Includes tooling-only import detection
...
- **Tooling-only import detection** identifies false orphans automatically

**Detection Methods:**
1. **Import Graph Analysis**: Real TypeScript import/export relationships via ts-morph
2. **Package.json Entrypoints**: Files invoked via npm/pnpm scripts are automatically detected
3. **Next.js Route Conventions**: Files like `page.tsx`, `layout.tsx`, `route.ts` are implicitly kept
4. **Barrel File Usage**: Index barrel files are kept if imported anywhere
5. **Text References**: Files referenced in docs/tests/scripts are marked for REVIEW
6. **Tooling-Only Detection**: Files only imported by tooling configs are flagged as warnings

**Tooling-Only Import Detection:**

The `validate:dead-code:optimized` command builds a full dependency graph (including tooling configs) to detect files that are only imported by tooling configuration files. These are "false orphans" because:

1. **Tooling configs are excluded from orphan scans** - They're consumed by CLI tools (Tailwind, PostCSS, etc.), not via TypeScript imports
2. **They show up as orphans** - Standard orphan detection doesn't see tooling config imports
3. **They're actually used** - The tooling configs import them at build time

**Detected Tooling Configs:**
...
- `vitest.config.ts/js` - Vitest test configuration
- `playwright.config.ts/js` - Playwright E2E test configuration
- `instrumentation.ts` / `instrumentation-client.ts` - Next.js instrumentation hooks

**Tooling-Only File Behavior:**
- ✅ **Shown as warnings (⚠️)**, not errors
- ✅ **Include import chain information** (shows which tooling config imports them)
- ✅ **Don't fail CI** - Only real orphans fail validation
- 💡 **Action**: If intentional, add to `scripts/audit/orphans.allowlist.json`
...
If `styles/breakpoints.ts` is only imported by `tailwind.config.ts` (and optionally tests), it will be detected as tooling-only and shown as a warning, not an error.
...
4. Check if file is a package.json script entrypoint
5. **Check if file is imported by tooling configs** (see tooling-only warnings)
6. Run typecheck and tests after removal
...
1. **Identify**: Run `pnpm validate:dead-code:optimized` for accurate list (includes tooling-only detection)
2. **Separate tooling-only from real orphans**:
   - ⚠️ Tooling-only imports (warnings) - Files only imported by tooling configs
   - ❌ Real orphans (errors) - Files with no imports at all
3. **Handle tooling-only files**:
   - Review each tooling-only file - check import chain information
   - If intentional (e.g., styles used by Tailwind config), add to `scripts/audit/orphans.allowlist.json`
   - If not needed, remove the import from tooling config and then remove the file
4. **Verify**: Check each real orphan for actual usage (not just scan scope)
5. **Remove**: Use `git rm` for tracked deletions
6. **Test**: Run typecheck + tests after each removal
...
**Cause:** Scan scope doesn't include all consumers, or tooling-only imports not detected
**Solution:**
- Current scan includes `app/` and excludes known false positives
- Use `validate:dead-code:optimized` (not `validate:orphans`) to detect tooling-only imports
- Check tooling-only warnings (⚠️) - these are false orphans and can be allowlisted
...
#### "File appears as orphan but is imported by tailwind.config.ts"
**Cause:** Tooling config imports are excluded from standard orphan scans
**Solution:**
- This is expected behavior - use `validate:dead-code:optimized` to detect tooling-only imports
- Tooling-only files are shown as warnings (⚠️) with import chain information
- If intentional, add to `scripts/audit/orphans.allowlist.json`
- Real orphans (❌) are files with no imports at all - these fail CI
...
- [Component Architecture](../architecture/codebase-structure.md) - Domain boundaries and import patterns
- [Development Setup Guide](../development/setup-guide.md) - Development environment and workflow including dependency management
- [Testing Strategy](../quality/testing-strategy.md) - Testing patterns for dependency changes
