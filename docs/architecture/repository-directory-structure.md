...
> ```bash
> pnpm tools:scan-dir . --max-depth 6 --exclude node_modules,.next,dist,coverage,.git,.cache,reports,test-results,test-reports > docs/architecture/repository-directory-structure.md
> ```
>
> **For codebase organization patterns and conventions**, see [Codebase Structure & Conventions](./codebase-structure.md).
