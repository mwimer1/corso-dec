...
│   └── server.ts        # Server-only utilities (auth() from @clerk/nextjs/server)
├── integrations/
...
import { auth } from '@clerk/nextjs/server';  // Use Clerk auth() directly
...
- Use domain-specific entry points and subpaths instead, for example:
  - `@/lib/auth/server` (server-only helpers; use `auth()` from `@clerk/nextjs/server` directly)
