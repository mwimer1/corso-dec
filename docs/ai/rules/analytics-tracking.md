...
title: "Rules"
---
# Analytics Tracking - Extended Documentation

This document contains provider integration details, testing patterns, and migration guides for analytics tracking. For the concise rule, see [`.cursor/rules/analytics-tracking.mdc`](../../../.cursor/rules/analytics-tracking.mdc).
...
export function trackNavClick(name: string, href: string) {
  ...
  try {
    ...
    // Log only in development (using publicEnv for client-safe access)
    // Note: In server code, use getEnv() from @/lib/server/env instead
