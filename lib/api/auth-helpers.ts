...
export async function requireAnyRoleForAI(
  ...
  // SECURITY: In production, always enforce RBAC (fail-closed)
  // Even if ENFORCE_AI_RBAC is set to false, production must enforce it
  if (isProduction) {
    if (!enforceRBAC) {
      // Log error but still enforce RBAC (fail-closed)
      logger.error('[AI RBAC] Attempted to disable RBAC in production - enforcing anyway', {
        route: routeName,
        userId: authContext.userId,
        orgId: authContext.orgId ?? null,
        code: 'RBAC_BYPASS_ATTEMPT',
      });
      // Continue to enforce RBAC below
    }
  } else {
    // Non-production: allow bypass if explicitly disabled (for testing)
    if (!enforceRBAC) {
      return authContext;
    }
  }
