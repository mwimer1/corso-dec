...
import { executeSqlAndFormat, executeSqlWithStructure, describeSchema, createDetectedTableIntent } from './tools';
  ...
  const readable = new ReadableStream({
    async start(controller) {
      try {
        ...
        let lastTabular: { columns: ColumnDef[]; data: Row[] } | null = null;
          ...
          if (choice.finish_reason === 'tool_calls' && functionCallId && functionName) {
            try {
              ...
              if (functionName === 'describe_schema') {
                ...
                for await (const contChunk of continuationStream) {
                  ...
                  if (contDelta?.content) {
                    ...
                    const contAiChunk: AIChunk = {
                      ...
                      detectedTableIntent: lastDetectedTableIntent,
                      error: null,
                    };
                  ...
                  if (contChoice.finish_reason === 'tool_calls' && functionCallId && functionName === 'execute_sql') {
                    ...
                    const sqlResult = await executeSqlWithStructure(args.query, orgId, req);
                    const sqlResults = sqlResult.formattedForModel;
                    const detectedTableIntent = createDetectedTableIntent(sqlResult.detectedTables);
                    
                    // Track tabular results
                    if (sqlResult.isTabular && sqlResult.rows.length > 0) {
                      lastTabular = {
                        columns: sqlResult.columns,
                        data: sqlResult.rows,
                      };
                      
                      // Emit table payload immediately
                      const tableChunk: AIChunk = {
                        assistantMessage: {
                          content: finalContent,
                          type: 'assistant',
                          visualizationType: 'table',
                          tableColumns: sqlResult.columns,
                          tableData: sqlResult.rows,
                        },
                        detectedTableIntent,
                        error: null,
                      };
                      controller.enqueue(encoder.encode(JSON.stringify(tableChunk) + '\n'));
                    }
                    ...
                    // Send final chunk with table payload if available
                    let assistantMsg: AssistantMessageChunk;
                    if (lastTabular) {
                      assistantMsg = {
                        content: finalContent,
                        type: 'assistant',
                        visualizationType: 'table',
                        tableColumns: lastTabular.columns,
                        tableData: lastTabular.data,
                      };
                    } else {
                      assistantMsg = {
                        content: finalContent,
                ...
                let assistantMsg: AssistantMessageChunk;
                if (lastTabular) {
                  const tabular = lastTabular as { columns: ColumnDef[]; data: Row[] };
                  assistantMsg = {
                    content: finalContent,
                    type: 'assistant',
                    visualizationType: 'table',
              ...
              if (functionName === 'execute_sql') {
                ...
                const sqlResult = await executeSqlWithStructure(args.query, orgId, req);
                const sqlResults = sqlResult.formattedForModel;
                lastDetectedTableIntent = createDetectedTableIntent(sqlResult.detectedTables);
        ...
        // No function call - send final chunk with table payload if available
        let assistantMsg: AssistantMessageChunk;
        if (lastTabular) {
          const tabular = lastTabular as { columns: ColumnDef[]; data: Row[] };
          assistantMsg = {
