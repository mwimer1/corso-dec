...
export async function streamResponseEvents(
  ...
  // Import executeSqlWithStructure and createDetectedTableIntent for structured results
  const { executeSqlWithStructure, createDetectedTableIntent } = await import('@/lib/api/ai/chat/tools');
  ...
  const executeSqlWithTracking = async (sql: string, orgId: string, req?: NextRequest): Promise<string> => {
    ...
    if (result.isTabular && result.rows.length > 0) {
      lastTabular = {
  ...
  while (true) {
    ...
    if (toolCallCount >= maxToolCalls) {
      ...
      let finalAssistantMsg: AssistantMessageChunk;
      if (lastTabular) {
    ...
    try {
      ...
      for await (const event of stream) {
        ...
        if (event.type === 'response.output_text.delta') {
          ...
          // Emit incremental text chunks with table payload if available
        ...
        for (const funcCall of pendingFunctionCalls) {
          ...
          try {
            ...
            if (funcCall.name === 'describe_schema') {
              ...
              if (args.query) {
                // Execute SQL function with structured tracking (pass req for logging context)
                const sqlResult = await executeSqlWithTracking(args.query, orgId, req);
      ...
      // Emit final chunk with table payload if available
      if (accumulatedText || lastTabular) {
        let finalAssistantMsg: AssistantMessageChunk;
        if (lastTabular) {
          const tabular = lastTabular as { columns: ColumnDef[]; data: Row[] };
          finalAssistantMsg = {
